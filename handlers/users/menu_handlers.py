...
async def list_categories(message: Union[CallbackQuery, Message], **kwargs):
    ...
    if isinstance(message, Message):
        ...
        await message.answer_photo(open('media/bear.jpg', 'rb'), caption="Смотри, что у меня есть", reply_markup=markup)
...
async def list_items(callback: CallbackQuery, category, subcategory, **kwargs):
    ...
    media = types.InputMediaPhoto(open('media/beer-belly.jpg', 'rb'), caption='Смотри, что у меня есть')
...
async def show_item(callback: CallbackQuery, category, subcategory, item_id):
    ...
    text = f"{item.name}"
    media = types.InputMediaPhoto(item.photo, caption=f"{item.name}\n{item.description}")
    await callback.message.edit_media(media=media, reply_markup=markup)
